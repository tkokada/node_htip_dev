  ...
  ifnames.forEach(function(dev) {
    ...
    try {
      ...
      session.on("packet", function(raw_packet) {
        ...
        // console.log(etherPacket);
        // console.log("ethertype: 0x" + etherPacket.ethertype.toString(16));
        // console.log("dhost: " + etherPacket.dhost);
        // console.log("shost: " + etherPacket.shost);
        mac = etherPacket.shost.toString();
        var lldp = etherPacket.payload;
        // console.log("lldp: " + lldp);
        for (i = 0; i < lldp.tlvs.length; i++) {
          ...
          case 0x7f: // Organazation specific
            ...
            case 0x02: // Link info
              nw = true;
              console.log(lldp.tlvs[i].payload.ttc_oui[0]);
              console.log(lldp.tlvs[i].payload.ttc_oui[1]);
              console.log(lldp.tlvs[i].payload.ttc_oui[2]);
              ...
              break;
            case 0x03: // MAC list
              console.log("--- HTIP MAC list handler has not been implemented yet." + mac + ", " + lldp.tlvs[i].payload.ttc_subtype);
              console.log(lldp);
              console.log(lldp.tlvs[i]);
              console.log(lldp.tlvs[i].payload);
              maclist = lldp.tlvs[i].payload.payload.payload;
              // ???
              /*
              maclist.some(function(v, i) {
                if (v === mac) {
                  maclist.slice(i, 1);
                }
              });
        ...
        // console.log("ip: " + ip + " mac: " + mac + " type: " + htip_type + " chassisid: " + chassis_id + " portid: " + port_id + " ttl: " + ttl + " port_desc: " + port_desc + " device_cat: " + device_cat + " manufacturer_code: " + manufacturer_code + " model_name: " + model_name + " model_number: " + model_number);
        var date = new Date();
        // console.log(date);
        if (ttl !== null) {
...
HtipReceiver.prototype.updateArp = function() {
  ...
  if (manager_ip_mac.length > 0) {
    ...
    // console.log("Manager ip: " + manager_ip + ", mac: " + manager_mac);
    node_list.add({
  ...
  upnp_devices.forEach(function(device) {
    ...
    request(device.headers.LOCATION, function(error, response, body) {
      if (error | response.statusCode !== 200) {
        return;
      }
      xml2js.parseString(body, function(err, result) {
        var fn = result.root.device[0].friendlyName;
        // console.log("friendlyName: " + fn);
